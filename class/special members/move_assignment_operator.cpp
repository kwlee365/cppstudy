...
// Update an object with a temporary object or a local object returned by value from a function
template <typename T>
SimpleVector<T>& SimpleVector<T>::operator=(SimpleVector<T>&& other) noexcept
{
    if(this != &other)
    {
        array = other.array;    // Move constructor에서는 initializer list에서 할당되었음.
        size = other.size;
        capacity = other.capacity;
